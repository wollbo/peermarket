...
def construct_url(
    url="https://api.tink.com/api/v1/oauth/token", ending="token", **kwargs
):
...
def create_bearer_token(base_url, **kwargs):
    ...
    assert str(request.status_code).startswith("2"), request.text
...
def payment_initiation(client_id, client_secret):
    ...
    data = {
        "destinations": [{"accountNumber": "SE2885222529285409533697", "type": "iban"}],
        "amount": 10,
        "currency": "SEK",
        "market": "SE",
...
if ACCOUNT:
    ...
    response = account_verification(
        TINK_CLIENT_ID, TINK_CLIENT_SECRET, ACCOUNT_VERIFICATION_REPORT_ID
    )
    print(response)
...
if WEBHOOK:
    ...
    TINK_CLIENT_SECRET = env_vars["ORACLEPAY_CLIENT_SECRET"]
    r = webhook_creation(
        TINK_CLIENT_ID, TINK_CLIENT_SECRET, "http://localhost:3000/webhook"
...
if PAYMENT:
    ...
    while True:  # or keeper polls this endpoint until status=sent
        ...
        payment_response = payment_status(
            TINK_CLIENT_ID, TINK_CLIENT_SECRET, json_response["id"]
        )
